    ...
    >
      <div className="max-w-7xl mx-auto px-6 md:px-8">
        <div className="flex items-center justify-between gap-8">
              ...
              {navItems.map((item) =>
                item.dropdown ? (
                  ...
                  >
                    <button
                      type="button"
                      className={`px-4 py-2 rounded-lg font-medium transition-all flex items-center gap-1 hover:-translate-y-0.5 ${
                  ...
                  <button
                    ...
                    className={`px-4 py-2 rounded-lg font-medium transition-all hover:-translate-y-0.5 whitespace-nowrap ${
                      isActive(item.path)
                        ? scrolled
                          ? 'bg-blue-900 text-white'
                          : 'bg-white/30 text-white'
          ...
          {/* Mobile Menu Button */}
          <button
            className={`lg:hidden transition-colors p-2 ${scrolled ? 'text-gray-900' : 'text-white'}`}
      ...
      {mobileMenuOpen && (
        <div className="lg:hidden bg-white/98 backdrop-blur-xl border-t border-gray-200 shadow-2xl">
          <div className="px-6 py-4 space-y-2">
              ...
              item.dropdown ? (
                ...
                <button
                  ...
                  className={`block w-full text-left py-3 px-4 rounded-lg font-medium transition-all ${
                    isActive(item.path) ? 'bg-blue-900 text-white' : 'text-gray-700 hover:bg-blue-50'
                  }`}
                >
                  {item.name}
                </button>
            ...
            <button
              onClick={() => handleNavClick('/contact')}
              className="block w-full text-center mt-4 bg-yellow-500 text-gray-900 px-6 py-3 rounded-lg font-semibold hover:bg-yellow-600 transition-all shadow-lg"
            >
              Get Started
            </button>
          </div>
